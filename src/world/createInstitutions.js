...
export function spawnInstitution(scene, shadows, type, point, state) {
  ...
  if (model) {
    ...
    switch (type) {
      case 'housing':   mat.diffuseColor = new Color3(0.56, 0.57, 0.6);  mesh = MeshBuilder.CreateBox(`housing-${Date.now()}`, { width: 14, depth: 14, height: 14 }, scene); mesh.position = new Vector3(point.x, 0.1, point.z); state.buildings.housing += 1; state.population += 120; state.legitimacy += 0.6; break;
      case 'school':    mat.diffuseColor = new Color3(0.52, 0.49, 0.43); mesh = MeshBuilder.CreateBox(`school-${Date.now()}`, { width: 18, depth: 16, height: 10 }, scene); mesh.position = new Vector3(point.x, 0.1, point.z); state.buildings.schools += 1; state.education += 3; state.approval += 1.2; break;
      case 'store':     mat.diffuseColor = new Color3(0.35, 0.37, 0.39); mesh = MeshBuilder.CreateBox(`store-${Date.now()}`, { width: 16, depth: 14, height: 8 }, scene); mesh.position = new Vector3(point.x, 0.1, point.z); state.buildings.stores += 1; state.cash += 180; break;
      case 'police':    mat.diffuseColor = new Color3(0.22, 0.25, 0.31); mesh = MeshBuilder.CreateBox(`police-${Date.now()}`, { width: 16, depth: 16, height: 10 }, scene); mesh.position = new Vector3(point.x, 0.1, point.z); state.buildings.police += 1; state.security += 4; break;
      case 'acc':       mat.diffuseColor = new Color3(0.4, 0.42, 0.45);  mesh = MeshBuilder.CreateBox(`acc-${Date.now()}`, { width: 16, depth: 16, height: 10 }, scene); mesh.position = new Vector3(point.x, 0.1, point.z); state.buildings.acc += 1; state.corruption -= 3; state.legitimacy += 2; break;
      case 'dec':       mat.diffuseColor = new Color3(0.31, 0.35, 0.36); mesh = MeshBuilder.CreateBox(`dec-${Date.now()}`, { width: 16, depth: 16, height: 10 }, scene); mesh.position = new Vector3(point.x, 0.1, point.z); state.buildings.dec += 1; state.security += 2; state.legitimacy += 1; break;
      case 'mine':      mat.diffuseColor = new Color3(0.38, 0.34, 0.31); mesh = MeshBuilder.CreateBox(`mine-${Date.now()}`, { width: 20, depth: 20, height: 8 }, scene); mesh.position = new Vector3(point.x, 0.1, point.z); state.buildings.mines += 1; state.steel += 8; break;
      case 'refinery':  mat.diffuseColor = new Color3(0.34, 0.37, 0.39); mesh = MeshBuilder.CreateBox(`refinery-${Date.now()}`, { width: 22, depth: 18, height: 10 }, scene); mesh.position = new Vector3(point.x, 0.1, point.z); state.buildings.refineries += 1; state.fuel += 6; break;
      case 'barracks':  mat.diffuseColor = new Color3(0.27, 0.31, 0.28); mesh = MeshBuilder.CreateBox(`barracks-${Date.now()}`, { width: 18, depth: 16, height: 8 }, scene); mesh.position = new Vector3(point.x, 0.1, point.z); state.buildings.barracks += 1; state.security += 3; break;
      case 'base':      mat.diffuseColor = new Color3(0.24, 0.28, 0.29); mesh = MeshBuilder.CreateBox(`base-${Date.now()}`, { width: 26, depth: 22, height: 10 }, scene); mesh.position = new Vector3(point.x, 0.1, point.z); state.buildings.bases += 1; state.security += 6; state.influence += 1; break;
      case 'stadium':   mat.diffuseColor = new Color3(0.48, 0.47, 0.44); mesh = MeshBuilder.CreateCylinder(`stadium-${Date.now()}`, { diameterTop: 18, diameterBottom: 22, height: 6, tessellation: 20 }, scene); mesh.position = new Vector3(point.x, 0.1, point.z); state.buildings.stadiums += 1; state.approval += 2; state.sportsPrestige += 3; break;
      default: return null;
    }
    mesh.material = mat;
