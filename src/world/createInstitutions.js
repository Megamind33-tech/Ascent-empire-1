  ...
  try {
    ...
    if (model) {
      ...
      switch (type) {
        ...
        default:
          console.error('[SpawnInstitution] Unknown building type:', type);
          return null;
      }

      // Only set material if mesh was created
      if (mesh) {
        mesh.material = mat;
    ...
    // Apply shadow setting only if mesh exists
    if (mesh) {
      mesh.receiveShadows = true;
      shadows.addShadowCaster(mesh);

      // Add to world meshes if state is valid
    ...
    console.error('[SpawnInstitution] Error spawning building:', err.message);
    return null;
  }
}