...
export function performAction(state, action){
  try {
    ...
    const placements = {
      buildHousing:['housing', c.housingCost],
      buildSchool:['school', c.schoolCost],
      buildStore:['store', c.storeCost],
      buildPolice:['police', c.policeCost],
      buildACC:['acc', c.accCost],
      buildDEC:['dec', c.decCost],
      buildMine:['mine', c.mineCost],
      buildRefinery:['refinery', c.refineryCost],
      buildBarracks:['barracks', c.barracksCost],
      buildBase:['base', c.baseCost],
      buildStadium:['stadium', c.stadiumCost]
    };
    ...
    if(action==='extinguish'){
      if (!state.worldRefs || !state.worldRefs.worldMeshes) {
        return fail('Game world not initialized. Cannot extinguish fire.');
      }
      const burning = state.worldRefs.worldMeshes.find(m => m?.metadata?.onFire);
      if(!burning) return fail('No active fires detected.');
      if(!spend(state, 800)) return fail('Not enough cash to deploy firefighters. (Cost: $800)');
      extinguishMesh(burning);
      return ok('🚒 Firefighters dispatched. Fire suppressed.');
    }

    if(placements[action]){
      const [mode,cost]=placements[action];
      if(!spend(state,cost)) return fail('Not enough cash.');
      state.selectionMode = mode;
      return ok('Tap a valid build pad to place it.');
    }

    if(action==='research'){
      if(!spend(state,c.researchCost)) return fail('Not enough cash to fund research.');
      state.research += 8;
      state.influence += 1.5;
      state.legitimacy += 1;
      return ok('Research funded.');
    }

    if(action==='campaign'){
      if(!spend(state,c.campaignCost)) return fail('Not enough cash to campaign.');
      state.influence += 6;
      state.approval += 2;
      return ok('Campaign launched.');
    }

    if(action==='travel'){
      return travelToNextNation(state);
    }
    ...
    console.error('[EconomySystem] Error during action:', action, err.message);
